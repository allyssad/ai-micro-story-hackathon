...
// hasn't been turned into training data yet
// neural net instantiated with values between 0-1
// would take neural net a substantial amount of time to ramp up and understand these larger numbers.
const rawData = [
  {
    date: '2018-11-02',
    open: 141.0716,
...
// we need to scale it down and normalize

const scaleDown = (step) => {
  // step in time
  // 138 is one of lowest values in rawData
  // to get these nums easily into NN is to simply divide by 138
  return {
    open: step.open / 138,
    high: step.high / 138,
    low: step.low / 138,
...
/* {
  open: 1.0222579710144928,
  high: 1.0224739130434783,
  low: 1.0056246376811593,
  close: 1.0129695652173913
} */

// de-normalization/scale up:
...
// scaling up the scaled down data (essentially back to it's starting point)
/* 
{
  open: 141.07160000000002,
  high: 141.1014,
  low: 138.7762,
  close: 139.7898
}
...
// bonus
// common approach to normalizing data:
// subtract lowest value from all other values
// then divide by the highest value - lowest value
...
// 140 - 138 = 2
// 147 - 138 = 9
// 2 / 9 = 0.222222 **IMPORTANT THAT VALUE IS BETWEEN 0 AND 1**

const normalize = (step) => {
  return {
    open: (step.open - 138) / (147 - 138),
    high: (step.high - 138) / (147 - 138),
    low: (step.low - 138) / (147 - 138),
    close: (step.close - 138) / (147 - 138),
  };
};

console.log(normalize(rawData[0]));
/*
{
  open: 0.3412888888888877,
  high: 0.34460000000000135,
  low: 0.08624444444444318,
  close: 0.1988666666666682
}
*/
